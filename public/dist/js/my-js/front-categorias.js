...
function actualizarCategoriaById(){
  ...
  let observacionesCategoria= document.getElementById('observacionesCategoriaActualizar').value;
   let idCategoria= document.getElementById('idCategoriaActualizar').value;
  $.ajax({
    method:'PUT',
    url: getURL()+"/categorias/api/"+idCategoria,
    data: {
...
function Generabotones(categoria){
  let Botones = '<div>';
                  Botones += '<button onclick="muestraUnaCategoria('+categoria.id+');" href="#" data-toggle="modal" data-target="#modal-actualizar" class="action-btn" type="button">'
		Botones += '<i class="fas fa-pencil-alt"></i>'
                  Botones += 'Actualizar'
                  Botones += '</button>'

                 
                  Botones += '<button onclick = "identificaIdEliminar('+categoria.id+')" href="#" data-toggle="modal" data-target="#deleteModal" class="action-btn" type="button">'
                  Botones += 'Eliminar'
                  Botones += '</button>'
                  Botones += '</div>';
  return Botones;
...
$.ajax({
  ...
  success: function( result ) {
    if(result.estado==1){
      ...
      let tabla = $('#tabla-categorias').DataTable();
      let botones = Generabotones(categoria);
      let nuevoRenglon = tabla.row.add([categoria.descripcion, categoria.observaciones, botones]).node();
